...
// .
// .
func TestAuditEchoReceiptUsesTheFinalSpanBoundary(t *testing.T) {
	...
	readTo := func(id string) []byte {
		...
		for {
			...
			if m.Params.Type == "synthesis_end" {
				ended = true
			}
			if string(m.ID) == id || (id == "event:synthesis_end" && ended) {
				return raw
			}
		}
	}
	...
	readTo("1")
	for _, fr := range []aiiosdk.AudioFrame{{Kind: aiiosdk.AudioPCM, Stream: 3, Seq: 1, PCM: make([]byte, 640)}, {Kind: aiiosdk.AudioDiscontinuity, Stream: 3, Seq: 2, Start: 1000}, {Kind: aiiosdk.AudioEnd, Stream: 3, Seq: 3, Start: 1000}} {
	...
	readTo("2")
	if !ended {
		readTo("event:synthesis_end")
	}
	if echoID == "" || echoStream == 0 {
		t.Fatal("echo was not named")
	}
	send(3, "speech.session.playback_report", fmt.Sprintf(`{"session_id":"s","synthesis_id":%q,"output_stream":%d,"rendered_samples":1000,"terminal":true}`, echoID, echoStream))
	reply := readTo("3")
	var receipt struct {
		Result struct {
